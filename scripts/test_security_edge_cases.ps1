...
# 1. AUTH
Log-Msg "1. Login as Admin..."
$token = (Invoke-RestMethod -Method Post -Uri "$BaseUrl/usuarios/login" -Body (@{login = "admin"; password = $env:ADMIN_PASSWORD } | ConvertTo-Json) -ContentType "application/json").token.Trim()
$headers = @{Authorization = "Bearer $token" }
